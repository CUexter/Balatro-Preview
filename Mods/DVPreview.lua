...
local settings_text = {
	"Enable Score Preview",
	"Enable Money Preview",
	"Show Min/Max Preview Instead of Exact",
	"Hide Preview if Any Card is Face-Down",
}
...
function create_UIBox_HUD()
	...
	local score_node_wrap =
		{ n = G.UIT.R, config = { id = "dv_pre_score_wrap", align = "cm", padding = 0.1 }, nodes = {} }
	...
	if G.SETTINGS.DV.preview_dollars then
		table.insert(dollars_node_wrap.nodes, DV.PRE.get_dollars_node())
	end
	table.insert(
		contents.nodes[1].nodes[1].nodes[5].nodes[2].nodes[3].nodes[1].nodes[1].nodes[1].nodes,
		dollars_node_wrap
	)
...
function G.UIDEF.settings_tab(tab)
	...
	function minmax_toggle_callback(_)
		if not G.HUD or not DV.PRE.enabled() then
			return
		end
	...
	local preview_score_toggle_label = G.localization.misc.dictionary["dv_settings_preview_score"] or settings_text[1]
	local preview_dollars_toggle_label = G.localization.misc.dictionary["dv_settings_preview_dollars"]
		or settings_text[2]
	local minmax_toggle_label = G.localization.misc.dictionary["dv_settings_show_min_max"] or settings_text[3]
	local face_down_toggle_label = G.localization.misc.dictionary["dv_settings_hide_face_down"] or settings_text[4]
	if tab == "Game" then
		local preview_setting_nodes = {
			n = G.UIT.R,
			config = { align = "cm" },
			nodes = {
				create_toggle({
					id = "score_toggle",
					label = preview_score_toggle_label,
...
function DV.PRE.get_score_node()
	...
	return {
		n = G.UIT.C,
		config = { id = "dv_pre_score", align = "cm" },
		nodes = {
			{
				n = G.UIT.O,
				config = {
					id = "dv_pre_l",
					func = "dv_pre_score_UI_set",
					object = DynaText({
						string = { { ref_table = DV.PRE.text.score, ref_value = "l" } },
			...
			},
			{
				...
				config = {
					...
					func = "dv_pre_score_UI_set",
					object = DynaText({
						string = { { ref_table = DV.PRE.text.score, ref_value = "r" } },
						colours = { G.C.UI.TEXT_LIGHT },
						shadow = true,
						float = true,
						scale = text_scale,
...
function DV.PRE.get_dollars_node()
	...
	return {
		n = G.UIT.C,
		config = { id = "dv_pre_dollars", align = "cm" },
		nodes = {
			{
				n = G.UIT.R,
				config = { align = "cm" },
				nodes = {
					{
						n = G.UIT.O,
						config = {
							id = "dv_pre_dollars_top",
							func = "dv_pre_dollars_UI_set",
							object = DynaText({
								string = { { ref_table = DV.PRE.text.dollars, ref_value = "top" } },
								colours = { top_color },
								shadow = true,
								spacing = 2,
								bump = true,
				...
				nodes = {
					{
						n = G.UIT.O,
						config = {
							id = "dv_pre_dollars_bot",
							func = "dv_pre_dollars_UI_set",
							object = DynaText({
								string = { { ref_table = DV.PRE.text.dollars, ref_value = "bot" } },
								colours = { bot_color },
								shadow = true,
								spacing = 2,
								bump = true,
